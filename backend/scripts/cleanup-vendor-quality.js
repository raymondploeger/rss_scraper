...
const client = new Client({
  ...
  application_name: "cleanup-vendor-quality",
});
...
function printSummary(candidates = []) {
  ...
  console.log("\n=== Vendor Quality Cleanup Summary ===");
  console.table(
    formatRows(
  ...
  console.table(
    formatRows(
      candidates.map((candidate) => ({
        ...
        url: candidate.url,
        rejection_reason: candidate.rejectedPageMatches.length
          ? candidate.rejectedPageMatches.join(", ")
          : candidate.matchedExclusions.length
...
async function deleteCandidates(candidates = []) {
  ...
  console.warn("This will permanently delete persisted Veridos and IN Groupe articles that fail current source relevance filters.");
...
async function main() {
  ...
  try {
    ...
    if (!apply) {
      ...
      console.log("Run with --apply to delete matching persisted vendor-quality articles.");
      return;
    }

    await deleteCandidates(candidates);
  } catch (error) {
    console.error("Failed to run vendor quality cleanup.");
