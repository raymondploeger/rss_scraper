...
function compareArticleEvidenceParity(article) {
  ...
  if ((legacyHardPassportNoise || legacyLowRelevancePassport) && !evidenceHasNoiseSignal) {
    parityWarnings.push("legacy_noise_guard_without_noise_evidence");
  }
...
function ensureFilterPipelineDiagnosticsExportTools() {
  ...
  if (typeof window.listRejectedArticles !== "function") {
    window.listRejectedArticles = () => listRejectedArticles();
  }
  ...
  if (typeof window.explainArticleDecisionByTitle !== "function") {
    window.explainArticleDecisionByTitle = (titlePart) => explainArticleDecisionByTitle(titlePart);
  }

  if (typeof window.findTracedArticlesByKeyword !== "function") {
  ...
  if (typeof window.listEvidenceParityWarnings !== "function") {
  ...
  if (typeof window.explainIdentityMatchByTitle !== "function") {
    window.explainIdentityMatchByTitle = (titlePart) => explainIdentityMatchByTitle(titlePart);
  }

  if (typeof window.listIdentityScoreTooLow !== "function") {
    window.listIdentityScoreTooLow = (limit) => listIdentityScoreTooLow(limit);
  }

  if (typeof window.listIdentityTechniqueBridgeMisses !== "function") {
...
function createFilterPipelineStageResult(stage, inputCount, outputCount, notes = [], warnings = []) {
  return {
    stage,
...
function renderArticles() {
  ...
  try {
    ...
    const activeFeedId = pipelineResult.activeFeedId || "";
    const activeFeedLabel = pipelineResult.activeFeedLabel || getSelectedFeedLabel();
    ...
    articles = filterPipelineResult.articles;
    personalDashboardBasePool = filterPipelineResult.candidatePool;
    filteredRawArticles = filterPipelineResult.filteredRawArticles;
    groupedArticlesCount = filterPipelineResult.groupedArticlesCount;
    feedDebugRawMatches = filterPipelineResult.feedDebugRawMatches;
    feedDebugAfterRelevanceMatches = filterPipelineResult.feedDebugAfterRelevanceMatches;
    feedRenderFilteredCount = filterPipelineResult.feedRenderFilteredCount;
    feedRenderGroupedCount = filterPipelineResult.feedRenderGroupedCount;
