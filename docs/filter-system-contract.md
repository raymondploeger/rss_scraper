...
The source panel now separates `No source filter` (clears a source selection) from `All tracked sources` (explicitly selects the full tracked set). For an active profile, both use the same cached candidate requests: targeted profile searches plus the recent baseline from every tracked-source group. This removes the former recent-only All path and keeps All a superset of each group; the profile/source audit also requires the unscoped profile and All counts to match. With no profile, the source-only count is labeled as recently loaded articles rather than a complete total. These requests are still bounded per query, so the feed is not a promise of exhaustive historical search.
