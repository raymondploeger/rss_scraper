...
The digital-identity domain's final decision now lives in `digital-identity-profile-policy.js`. For Identity Verification, accepted articles therefore no longer pass through the legacy fallback. Shared upstream domain gates and professional-noise rejections are still in the old matcher and remain reported as fallback rejections. The route audit asserts this narrower migration boundary while the profile/source and interest audits protect visible results.
