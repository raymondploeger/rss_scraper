...
The source panel has one `All sources` choice, selected by default. Choosing it clears a group, specific feed, or DMV source selection. With an active profile it uses targeted profile searches plus the recent baseline from every tracked-source group, and returning from a group to All must restore the same count. This removes the former recent-only All path and keeps All a superset of each group. With no profile, the source-only count is labeled as recently loaded articles rather than a complete total. These requests are still bounded per query, so the feed is not a promise of exhaustive historical search.
