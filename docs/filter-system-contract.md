...
`profile-mode-policy.js` records the legacy general scoring modes separately from Identity Document Authority strictness. The Authority has a user-selectable Strict, Standard, or Expanded profile strictness. Strict requires direct identity-document authority evidence; Standard also admits relevant government identity-document context; Expanded also admits adjacent civil-identity and authority context. Other profiles retain their existing Balanced scoring default; because the interface provides no control for it, the feed and article explanation do not display a mode badge for those profiles.
...
The source panel has one `All sources` choice, selected by default. Choosing it clears a group, specific feed, or DMV source selection. With an active profile, each source-scoped baseline explicitly requests a complete candidate pool: the backend deduplicates every matching stored article and the frontend loads every returned page before applying the profile policy. Returning from a group to All must restore the same count, and All remains a superset of each group. With no profile, the source-only count is labeled as recently loaded articles rather than a complete total. An active profile therefore evaluates all articles currently stored in its selected tracked-source scope; a date filter still supplies the explicit time bound when one is selected.
